...
| `frida_run <package> <script.js \| --preset NAME> [--spawn] [--arg key=val ...]` / `frida_run --list-presets` | Inject a Frida JS script via on-device `frida-server`. `--arg` injects a `const _args = {...}` prelude that parameterized presets read. Bundled presets ship under `commands/android/frida_presets/` (`ssl_pinning_bypass`, `root_bypass`, `biometric_bypass`, `intent_spy`, `http_capture`, `keystore_inventory`, `crypto_intercept`, `method_trace`, `native_enum`, `antidebug_bypass`, `native_trace`). |
...
| `mastg_report <package> [--out FILE] [--json \| --html] [--secrets-dir DIR]` | Composite MASTG-aligned static report (app_scan + app_provider_probe + app_deeplinks + optional secrets), grouped by MASVS category with MSTG test IDs. `--html` emits a self-contained styled page with `<details>` collapsibles and severity badges. |
| `mastg_full <package> [--out DIR] [--decompiled DIR] [--skip-pull] [--skip-decompile]` | End-to-end macro: app_pull → app_decompile → every static scanner → mastg_report MD + HTML → `index.html` linking all artefacts. |
| `app_dex_dump <package> [--out DIR] [--spawn] [--seconds N]` | Frida-driven DEX dump from memory (scans `r--` ranges for DEX magic, writes each to `dex_<base>_<size>.dex`). Catches runtime unpackers (Bangcle / Tencent Legu / generic decrypt-on-class-load). |
