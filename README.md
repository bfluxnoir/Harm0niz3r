...
| `frida_run <package> <script.js \| --preset NAME> [--spawn]` / `frida_run --list-presets` | Inject a Frida JS script via on-device `frida-server`. Bundled presets ship under `commands/android/frida_presets/` (`ssl_pinning_bypass`, `root_bypass`, `biometric_bypass`, `intent_spy`, `http_capture`). |
